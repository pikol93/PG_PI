...
class ActiveSessionBottomSheetShown extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) => Column(
        ...
        children: [
          ...
          _Header(activeSession: activeSession),
          ElevatedButton(
            onPressed: () => ref
...
class _Header extends ConsumerWidget with Logger {
  final ActiveSession activeSession;

  _Header({required this.activeSession});
  ...
  Future<void> _onEndExercisePressed(WidgetRef ref) async {
    ...
    await ref
        .read(activeSessionServiceProvider)
        .update(
          activeSession.copyWith(currentExerciseIndex: const Option.none()),
        )
        .run();
  }
